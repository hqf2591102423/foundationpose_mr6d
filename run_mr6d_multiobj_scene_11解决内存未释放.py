...
def build_estimator(mesh, debug_dir, debug, scorer, refiner, glctx):
...
if __name__ == '__main__':
    ...
    # 为了避免每次换物体都重新加载庞大的神经网络导致显存泄漏，
    # 我们在最外层初始化它们，然后在 build_estimator 中复用
    logging.info("Initializing global models (ScorePredictor, PoseRefinePredictor)...")
    scorer = ScorePredictor()
    refiner = PoseRefinePredictor()
    glctx = dr.RasterizeCudaContext()

    # 自动解析每个物体的首次出现帧，而不再假定所有物体都从第 0 帧开始
    targets = []
    seen_inst_indices = set()
    
    for seq_idx, img_id in enumerate(scene.img_ids):
        for inst_idx, obj in enumerate(scene.scene_gt[img_id]):
            # 假定同一场景中基于列表索引 inst_idx 能唯一追踪一个物体实例
            if inst_idx not in seen_inst_indices:
                seen_inst_indices.add(inst_idx)
                targets.append({
                    'inst_idx': inst_idx,
                    'obj_id': obj['obj_id'],
                    'first_img_id': img_id,
    ...
    for t_info in targets:
        inst_idx = t_info['inst_idx']
        obj_id = t_info['obj_id']
        start_seq_idx = t_info['start_seq_idx']
        first_img_id = t_info['first_img_id']

        logging.info(f"Initializing obj_id={obj_id} inst_idx={inst_idx} starting at frame {start_seq_idx} (img_id={first_img_id})")
        ...
        est = build_estimator(mesh, obj_debug_dir, args.debug, scorer, refiner, glctx)
        logging.info("estimator initialization done")

        # 从该物体首次出现的帧开始跟踪
        for i in range(start_seq_idx, len(scene.img_ids)):
            img_id = scene.img_ids[i]
            logging.info(f'obj_id={obj_id} inst_idx={inst_idx} frame={i} (img_id={img_id})')
            color = scene.get_color(img_id)
            depth = scene.get_depth(img_id)
            K = scene.get_K(img_id)

            # 现在每一帧都使用其对应的 mask 调用 register 做检测，而不使用 track_one
        ...
        # 在处理下一个物体前清理显存显式释放之前的 estimator 和网络模型
        del est
        import torch
