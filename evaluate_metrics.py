...
def evaluate(gt_json, pred_json, models_dir, out_json=None):
    ...
    if out_json:
        result_to_save = {
            'global': {
                'instances': len(metrics['ADD']),
                'ADD': metrics['ADD'],
                'e_R': metrics['e_R'],
                'e_t': metrics['e_t'],
                'ADD_lt_0.1d': metrics['ADD_lt_0.1d']
            },
            'per_object': {
                int(k): v for k, v in obj_metrics.items()
            }
        }
        with open(out_json, 'w') as f:
            json.dump(result_to_save, f, indent=4)
        print(f"Saved exact raw metrics into {out_json}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_json', type=str, default=None, help='Save the raw metrics array to a json file')
    args = parser.parse_args()
    
    evaluate(args.gt_json, args.pred_json, args.models_dir, args.out_json)
